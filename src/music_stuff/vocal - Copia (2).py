...
class Vocal(commands.Cog):
    ...
    async def play(self, ctx: commands.Context, *, search):
        ...
        html = urllib.request.urlopen(f"https://www.youtube.com/results?search_query={yt_search}")
        video_ids = re.findall(r"watch\?v=(\S{11})", html.read().decode())
        await ctx.send(f"http://www.youtube.com/watch?v={video_ids[0]}")
        
    
        
        
        await ctx.send(f"Downloading {search}...")
        
        
        ydl_opts = {
            'format': 'bestaudio/best',
            'outtmpl': 'src/songs/song.mp3',
            'postprocessors': [{
        ...
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            ydl.download([f"http://www.youtube.com/watch?v={video_ids[0]}"])

        
        audio_source = discord.FFmpegPCMAudio('songs/song.mp3')
        await ctx.voice_client.play(audio_source, after=None)
        os.remove("src/songs/song.mp3")
